...
from models import Signal, Candle, IndicatorValues, OptionType
from indicators import calculate_all_indicators, get_latest_indicators
from candle_builder import CandleBuilder
from utils import logger, is_market_hours, calculate_atm_strike
...
class AlphaEngine:
    ...
    async def process_candle(self, candle: Candle) -> Optional[Signal]:
        ...
        if not self.candle_builder.has_enough_data:
            logger.debug(f"Waiting for more data: {self.candle_builder.candle_count} candles")
            return None
            ...
            while self._running:
                try:
                    ...
                    if signal and signal != Signal.HOLD:
                        ...
                        atm_strike = calculate_atm_strike(spot_price)
                        option_type = self.alpha_engine.get_option_type_for_signal(signal)
